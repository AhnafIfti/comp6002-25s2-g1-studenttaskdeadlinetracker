...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!token && !isRegisterPage) {
      // Prevent redirection if on '/register'
      ...
      <main className={`main-content ${!isAuthenticated ? "full-width" : ""}`}>
        <Routes>
          ...
          <Route
            path="/group-tasks"
            element={
              isAuthenticated ? <GroupTasks /> : <Navigate to="/login" />
            }
          />
          <Route
            path="/deadline"
            element={isAuthenticated ? <SubTasks /> : <Navigate to="/login" />}
          />
          <Route
            path="/analytics"
          ...
          <Route
            path="/calendar"
            element={
              isAuthenticated ? <TaskCalendar /> : <Navigate to="/login" />
            }
          />
          <Route
            path="/profile"
...
const App: React.FC = () => {
  const GOOGLE_CLIENT_ID =
    "329869064237-kclvre8bq3sdnee4s6vn4a22vr2nj076.apps.googleusercontent.com";
